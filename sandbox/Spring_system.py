...
def calculate_total_energy(system, T): #T = temperature (K)
    k_B = 1.38065E-23 
    total_energy = 0
    ...
    for beam in system:
        if isinstance(beam, Beam):
            if (beam.node_1.orientation_node == False and beam.node_2.orientation_node == False) and (beam.node_1.dsDNA and beam.node_2.dsDNA): 
                # both n1 and n2 are backbone nodes and dsDNA
                L_0 = 0.34 # nm
                k_dsDNA = 2.94 # J/m^2
                energy = calculate_bond_energy(beam.node_1.position, beam.node_2.position, k_dsDNA, L_0)
                total_energy += energy
            elif beam.node_1.orientation_node and beam.node_2.dsDNA:
                # Energy of orientation node connected to a real node
                assert (beam.node_1.orientation_node and beam.node_2.dsDNA), "Only dsDNA have orientation beads"
                L_0 = 0.15 #nm
                k_orient = 20.8 #J/m^2
                energy = calculate_bond_energy(beam.node_1.position, beam.node_2.position, k_orient, L_0)
                total_energy += energy
            elif (beam.node_1.orientation_node == False and beam.node_2.orientation_node == False) and (beam.node_1.dsDNA == False and beam.node_2.dsDNA == False):
                # both nodes are ssDNA 
                L_0 = 0.64 # nm
                k_ssDNA = 2.352 # J/m^2
                energy = calculate_bond_energy(beam.node_1.position, beam.node_2.position, k_ssDNA, L_0)
                total_energy += energy
    return total_energy
...
print(calculate_total_energy(test_system, 298))
